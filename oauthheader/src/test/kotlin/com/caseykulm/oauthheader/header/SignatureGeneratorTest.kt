...
class SignatureGeneratorTest {
  ...
  val oauthStage = OauthStage.GET_RESOURCE
  ...
  fun getSignatureBaseString() {
    assertEquals(
        ...
        siggyGen.getBaseString(getNonceGenerator().generate(), getCalendar().utcTimeStamp(), getResourceRequest(), oauthStage, accessToken))
  }
  ...
  fun getSignatureBaseStringParamsPart() {
    assertEquals(
        ...
        siggyGen.getParamsEncodedString(
            getNonceGenerator().generate(),
            getCalendar().utcTimeStamp(),
            getResourceRequest(),
            oauthStage,
            accessToken))
  }
  ...
  @Test
  fun getSiggy() {
    assertEquals("tnnArxj06cWHq44gCs1OSKk%2FjLY%3D", siggyGen.getSignatureEncoded(getNonceGenerator().generate(), getCalendar().utcTimeStamp(), getResourceRequest(), oauthStage, accessToken, accessSecret))
  }
